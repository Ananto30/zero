"""
Benchmark truly concurrent async - using tasks to decouple recv from send.
"""

import asyncio
import logging
import multiprocessing
import time

import zmq
import zmq.asyncio

logging.basicConfig(level=logging.WARNING)

PORT_SYNC = 5556
PORT_ASYNC_SEQUENTIAL = 5557
PORT_ASYNC_CONCURRENT = 5558
HOST = "127.0.0.1"
NUM_CLIENTS = 10


# ============================================================================
# SYNC VERSION (sequential)
# ============================================================================
def sync_server():
    """Sync echo server - handles clients sequentially"""
    context = zmq.Context()
    socket = context.socket(zmq.REP)
    socket.bind(f"tcp://{HOST}:{PORT_SYNC}")
    print("[SYNC] Server started")

    count = 0
    try:
        while True:
            message = socket.recv()
            socket.send(message)
            count += 1
            if count % 500 == 0:
                print(f"[SYNC] Server: {count} requests")
    except KeyboardInterrupt:
        pass
    finally:
        socket.close()
        context.term()


# ============================================================================
# ASYNC SEQUENTIAL (old way - blocks on send)
# ============================================================================
async def async_server_sequential():
    """Async but sequential - waits for send to complete before next recv"""
    context = zmq.asyncio.Context()
    socket = context.socket(zmq.REP)
    socket.bind(f"tcp://{HOST}:{PORT_ASYNC_SEQUENTIAL}")
    print("[ASYNC-SEQ] Server started")

    count = 0
    try:
        while True:
            message = await socket.recv()
            await socket.send(message)  # BLOCKS loop until send completes
            count += 1
            if count % 500 == 0:
                print(f"[ASYNC-SEQ] Server: {count} requests")
    except KeyboardInterrupt:
        pass
    finally:
        socket.close()
        context.term()


# ============================================================================
# ASYNC CONCURRENT (new way - tasks decouple recv from send)
# ============================================================================
async def send_handler(socket, message):
    """Handle sending response asynchronously"""
    await socket.send(message)


async def async_server_concurrent():
    """Truly concurrent async - recv doesn't wait for send"""
    context = zmq.asyncio.Context()
    socket = context.socket(zmq.REP)
    socket.bind(f"tcp://{HOST}:{PORT_ASYNC_CONCURRENT}")
    print("[ASYNC-CONC] Server started")

    count = 0
    try:
        while True:
            message = await socket.recv()
            # Create task but don't await - loop immediately goes back to recv()
            asyncio.create_task(send_handler(socket, message))
            count += 1
            if count % 500 == 0:
                print(f"[ASYNC-CONC] Server: {count} requests")
    except KeyboardInterrupt:
        pass
    finally:
        socket.close()
        context.term()


def async_server_sequential_process():
    asyncio.run(async_server_sequential())


def async_server_concurrent_process():
    asyncio.run(async_server_concurrent())


# ============================================================================
# CONCURRENT CLIENT
# ============================================================================
def client_worker(port: int, client_id: int, num_requests: int = 1000):
    """Single client sending requests"""
    context = zmq.Context()
    socket = context.socket(zmq.REQ)
    socket.setsockopt(zmq.RCVTIMEO, 5000)
    socket.connect(f"tcp://{HOST}:{port}")

    for i in range(num_requests):
        try:
            socket.send(b"test")
            response = socket.recv()
        except zmq.error.Again:
            print(f"Client {client_id} timeout after {i} requests")
            break

    socket.close()
    context.term()


def run_concurrent_benchmark(port: int, num_clients: int, num_requests_per_client: int):
    """Run multiple clients concurrently"""
    procs = []

    start = time.time()

    for i in range(num_clients):
        proc = multiprocessing.Process(
            target=client_worker, args=(port, i, num_requests_per_client)
        )
        proc.start()
        procs.append(proc)

    for proc in procs:
        proc.join()

    elapsed = time.time() - start
    total_requests = num_clients * num_requests_per_client
    rps = total_requests / elapsed

    return rps, elapsed


# ============================================================================
# MAIN
# ============================================================================
if __name__ == "__main__":
    num_clients = NUM_CLIENTS
    num_requests_per_client = 1000
    total_requests = num_clients * num_requests_per_client

    print(f"\n{'='*60}")
    print(f"SYNC SERVER BENCHMARK: {num_clients} clients, {num_requests_per_client} req/client")
    print(f"Total: {total_requests} requests")
    print(f"{'='*60}")

    sync_server_proc = multiprocessing.Process(target=sync_server)
    sync_server_proc.daemon = True
    sync_server_proc.start()

    time.sleep(0.5)

    sync_rps, sync_time = run_concurrent_benchmark(PORT_SYNC, num_clients, num_requests_per_client)
    print(f"Sync RPS: {sync_rps:.2f} req/sec")
    print(f"Total time: {sync_time:.2f}s\n")

    sync_server_proc.terminate()
    sync_server_proc.join(timeout=2)

    time.sleep(2)

    print(f"\n{'='*60}")
    print(
        f"ASYNC SEQUENTIAL BENCHMARK: {num_clients} clients, {num_requests_per_client} req/client"
    )
    print(f"Total: {total_requests} requests")
    print(f"{'='*60}")

    async_seq_proc = multiprocessing.Process(target=async_server_sequential_process)
    async_seq_proc.daemon = True
    async_seq_proc.start()

    time.sleep(0.5)

    async_seq_rps, async_seq_time = run_concurrent_benchmark(
        PORT_ASYNC_SEQUENTIAL, num_clients, num_requests_per_client
    )
    print(f"Async Sequential RPS: {async_seq_rps:.2f} req/sec")
    print(f"Total time: {async_seq_time:.2f}s\n")

    async_seq_proc.terminate()
    async_seq_proc.join(timeout=2)

    time.sleep(2)

    print(f"\n{'='*60}")
    print(
        f"ASYNC CONCURRENT BENCHMARK: {num_clients} clients, {num_requests_per_client} req/client"
    )
    print(f"Total: {total_requests} requests")
    print(f"{'='*60}")

    async_conc_proc = multiprocessing.Process(target=async_server_concurrent_process)
    async_conc_proc.daemon = True
    async_conc_proc.start()

    time.sleep(0.5)

    async_conc_rps, async_conc_time = run_concurrent_benchmark(
        PORT_ASYNC_CONCURRENT, num_clients, num_requests_per_client
    )
    print(f"Async Concurrent RPS: {async_conc_rps:.2f} req/sec")
    print(f"Total time: {async_conc_time:.2f}s\n")

    async_conc_proc.terminate()
    async_conc_proc.join(timeout=2)

    print(f"\n{'='*60}")
    print(f"RESULTS - Multiple Concurrent Clients")
    print(f"{'='*60}")
    print(f"Sync (Sequential):        {sync_rps:.2f} req/sec  ({sync_time:.2f}s total)")
    print(f"Async (Sequential):       {async_seq_rps:.2f} req/sec  ({async_seq_time:.2f}s total)")
    print(f"Async (Concurrent Tasks): {async_conc_rps:.2f} req/sec  ({async_conc_time:.2f}s total)")
    print(f"\nComparisons:")
    print(f"Async Concurrent vs Sync:          {async_conc_rps / sync_rps:.2f}x")
    print(f"Async Concurrent vs Async Seq:     {async_conc_rps / async_seq_rps:.2f}x")
    print(f"Time saved (Async Conc vs Async Seq): {async_seq_time - async_conc_time:.2f}s")
    print(f"{'='*60}\n")
