...
class ZeroServer:
    ...
    def _start_queue_device(self):
        try:
            ...
            poll_both.register(gateway, zmq.POLLIN)
            ...
            while True:
                ...
                if socks.get(backend) == zmq.POLLIN:
                    ...
                    if reply[0] != LRU_READY:
                        gateway.send_multipart(reply)
                if socks.get(gateway) == zmq.POLLIN:
                    # Get client request, route to first available worker
                    msg = gateway.recv_multipart()
...
class _Worker:
    ...
    def start_dealer_worker(self, worker_id):
        ...
        if os.name == "posix":
            worker.connect(f"ipc://{self._ipc}")
        else:
            worker.connect(f"tcp://127.0.0.1:{self._port}")
        logging.info(f"Starting worker: {worker_id}")
        worker.send_string(LRU_READY)

        def process_message():
            try:
                ident, e, rpc, msg = worker.recv_multipart()
                
                rpc_method = rpc.decode()
                msg = self._decode(msg)
                response = self._handle_msg(rpc_method, msg)
                response = self._encode(response)
                worker.send_multipart([ident,e, response], zmq.DONTWAIT)
            except Exception as e:
                logging.exception(e)
