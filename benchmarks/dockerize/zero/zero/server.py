...
from .zero_mq import AsyncZeroMQ, ZeroMQ
...
class AsyncZeroServer:
    def __init__(self, host: str = "0.0.0.0", port: int = 5559):
        """
        AsyncZeroServer registers rpc methods that are called from a ZeroClient.

        By default AsyncZeroServer uses all of the cores for best performance possible.
        A zmq queue device load balances the requests and runs on the main thread.
    ...
    def register_rpc(self, func: typing.Callable):
        ...
        if not inspect.iscoroutinefunction(func):
            raise Exception(f"register function; not coroutine function")
        if func.__name__ in self._rpc_router:
            raise Exception(f"Cannot have two RPC function same name: `{func.__name__}`")
        if func.__name__ == "get_rpc_contract":
            raise Exception("get_rpc_contract is a reserved function; cannot have `get_rpc_contract` as a RPC function")
...
class _AsyncWorker:
    ...
    def spawn_worker(
        ...
        time.sleep(0.2)
        worker = _AsyncWorker(rpc_router, ipc, port, serializer, rpc_input_type_map, rpc_return_type_map)
    ...
    async def start_dealer_worker(self, worker_id):  # pragma: no cover
        async def process_message(rpc, msg):
            try:
                rpc_method = rpc.decode()
                msg = self._decode(msg)
        ...
        await AsyncZeroMQ.worker(self._ipc, self._port, worker_id, process_message)
