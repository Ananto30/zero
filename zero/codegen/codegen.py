...
class CodeGen:
    ...
    def generate_code(
        self, host: str, port: int, protocol: str, async_client: bool
    ) -> str:
        if protocol not in ("zmq", "tcp"):
        ...
        if protocol == "tcp":
            ...
            client_class_name = "AsyncZeroClient"
            client_init = f'zero_client = AsyncZeroClient("{host}", {port})\n\n'
            is_async = True
        else:
            client_import = "from zero import ZeroClient\n\n\n"
            client_class_name = "ZeroClient"
            client_init = f'zero_client = ZeroClient("{host}", {port})\n\n'
            is_async = False

        code = f"""# Generated by Zero
# import types as per needed, modify as per your project structure

{client_import}{client_init}"""
        code += self.generate_models()
        code += f"""
class RpcClient:
    def __init__(self, zero_client: {client_class_name}):
...
"""
    ...
    def get_imports(self, code: str) -> str:
    ...
    def _track_imports(self, input_type: Optional[Type]) -> None:
        if not input_type:
            return
    ...
    def get_function_str(self, func_name: str, is_async: bool) -> str:
    ...
    def get_function_input_param_name(self, func_name: str) -> str:
        func = self._rpc_router[func_name][0]
        func_lines = inspect.getsourcelines(func)[0]
        func_str = "".join(func_lines)
    ...
    def _resolve_field_type(self, field_type: Type) -> List[Type]:
